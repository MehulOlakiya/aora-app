...
export const getBookmarkVideo = async (userId)  => {
  try {
    const bookMarkPost = await database.listDocuments(databaseId,userCollectionId,[Query.equal('$id',userId)])
    return bookMarkPost.documents[0].savedVideo
    // console.log('test',JSON.stringify(bookMarkPost.documents[0].savedVideo,null,2))
  } catch (error) {
    throw new Error(error)
  }
}
